...
# ── Fusion-module interface (capability-gated) ───────────────────────────────
# Requires SKIN_TEMP. The registry builds this module only when the device
# carries a peripheral temperature sensor; otherwise it is never instantiated.

from affectus.config import TEMP_CHANNEL_MAX_WEIGHT  # noqa: E402
from affectus.contract.capabilities import Capability  # noqa: E402
from affectus.shared.fusion import FusionChannel  # noqa: E402

REQUIRES = frozenset({Capability.SKIN_TEMP})
NAME = "temp"
...
def evaluate(ctx) -> FusionChannel | None:
    ...
    if weight <= 0.0:
        return None
    return FusionChannel(NAME, z=ev.z, weight=weight, confidence=ev.confidence)
