...
Each device family registers its own profile via register_profile() from
devices/<family>/profile.py. This module owns only the structure + lookup; the
values live with the device that owns them.
"""
...
def profile_for(name: str) -> SensorProfile:
    ...
    Device families are imported lazily here so registration happens before the
    first lookup without creating an import cycle (shared/ must not import
    devices/ at module load)."""
    if not _PROFILES:
        import affectus.devices  # noqa: F401  (triggers family registration)
