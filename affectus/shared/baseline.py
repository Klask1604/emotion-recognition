...
class RestBaselineStore:
    ...
    def observe_resting(
        ...
        *,
        ...
        Samples are accepted at most once per `min_spacing_s` (default
        BASELINE_OBSERVATION_INTERVAL_S) so the locked baseline is built from
        less-correlated points: the 60 s HRV window means consecutive ~1 Hz calls
        share almost all their beats, which collapses the MAD and makes the
        personal z hypersensitive. A clean ECG calibration can pass a smaller
        spacing to lock faster (its IBI is gold-standard, less correlated noise).
        `now` is the wall-clock in seconds; when omitted the spacing gate is
        disabled (kept for tests that drive the store directly)."""
    ...
    def reset_for_recalibration(self, reported_arousal: float | None = None) -> None:
        """Only call from affectus/cmd/calibrate. `reported_arousal` (0..1) is the
    ...
    def arousal_offset_z(self) -> float:
        ...
        from affectus.shared.arousal_mapper import normal_ppf
