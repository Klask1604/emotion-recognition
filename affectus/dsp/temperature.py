...
# The fusion-channel wrapper (REQUIRES / evaluate) that used to live here now sits
# in engine/channels.py alongside the HRV and HR channel builders, so all three
# channels are assembled in one readable place. This module keeps only the
# temperature MATH (state + evaluate_skin_temperature), which the pipeline owns.
